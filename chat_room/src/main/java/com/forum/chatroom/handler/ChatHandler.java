...
public class ChatHandler implements WebSocketHandler {
	...
	public Mono<Void> handle(WebSocketSession webSocketSession) {
		...
		Mono<Void> sendMono = webSocketSession.send(outFlux).doFinally(signalType -> {
		...
		// 把两个mono 的消息汇总起来 再返回
		return Mono.when(mono1, mono2).then(sendMono);
	...
	private void sendClientCountToAllSessions(WebSocketSession webSocketSession) {
		...
		if (webSocketSession.isOpen()) {
			webSocketSession.send(Mono.just(webSocketSession.textMessage(currentClientCount))).subscribe();
		}
		// 遍历所有客户端并发送当前的客户端连接数
		for (Map.Entry<WebSocketSession, FluxSink<WebSocketMessage>> entry : MY_CLIENTS.entrySet()) {
